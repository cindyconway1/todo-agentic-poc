...
/// <summary>
/// DB-free smoke tests for the /api/teams wiring (BE-04): routing, the AuthenticatedUser policy,
...
/// </summary>
public class TeamsPipelineSmokeTests : IClassFixture<WebApplicationFactory<Program>>
{
    private readonly WebApplicationFactory<Program> _factory;

    public TeamsPipelineSmokeTests(WebApplicationFactory<Program> factory) => _factory = factory;
    ...
    [Fact]
    public async Task ListTeams_WhenUnauthenticated_Returns401_NotRedirect()
    {
        var response = await CreateClient().GetAsync("/api/teams");

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task GetTeam_WhenUnauthenticated_Returns401()
    {
        var response = await CreateClient().GetAsync($"/api/teams/{Guid.NewGuid()}");

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task CreateTeam_WhenUnauthenticated_Returns401()
    {
        var response = await CreateClient().PostAsync("/api/teams", JsonBody("{\"name\":\"Smoke Team\"}"));

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task UpdateTeam_WhenUnauthenticated_Returns401()
    {
        var response = await CreateClient().PutAsync($"/api/teams/{Guid.NewGuid()}", JsonBody("{\"name\":\"Smoke Team\"}"));

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task DeleteTeam_WhenUnauthenticated_Returns401()
    {
        var response = await CreateClient().DeleteAsync($"/api/teams/{Guid.NewGuid()}");

        Assert.Equal(HttpStatusCode.Unauthorized, response.StatusCode);
    }

    [Fact]
    public async Task CreateTeam_Authenticated_WithoutAntiforgeryToken_IsRejectedCleanly()
    {
        var response = await CreateAuthenticatedClient().PostAsync("/api/teams", JsonBody("{\"name\":\"Smoke Team\"}"));
    ...
    // Regression anchor: with a non-nullable Name on CreateTeamRequest, [ApiController]
    // implicit-required validation would return 400 before TeamEdit's Required rule ran, so the
    // contractual 422 path would never be reached. DB-free because TeamEdit's [Create] never touches
    // SQL — the request fails validation and returns before any save.
    [Fact]
    public async Task CreateTeam_WithNullName_Returns422FromBusinessRule_Not400FromAutoValidation()
    {
    ...
    private sealed class TestAuthHandler : AuthenticationHandler<AuthenticationSchemeOptions>
    {
        ...
        private static readonly Guid TestUserId = Guid.Parse("6f1a3c5e-0000-4000-8000-000000000002");
